...
def fetch_new_data_package_views(model):
    ...
    FROM package pck
        JOIN _org_hierarchy orc ON pck.owner_org = orc.org_id
        JOIN tracking_summary tms ON tms.package_id = pck.id
    WHERE pck.state = 'active'::text AND tms.tracking_date > COALESCE((SELECT tracking_date FROM public.ckanext_opendstats_package_views order by tracking_date DESC limit 1),'1970-01-01');
    '''
    conn = model.Session.connection()
    try:
        conn.execute(sa.sql.text(sql))
...
def fetch_new_data_resource_downloads(model):
    ...
    FROM resource rcs
        ...
        JOIN _org_hierarchy orc ON pck.owner_org = orc.org_id
        JOIN tracking_summary tms ON tms.tracking_type::text = 'resource'::text
    WHERE pck.state = 'active'::text AND tms.url ~~ (('%'::text || rcs.id) || '/download%'::text) AND tms.tracking_date > COALESCE((SELECT tracking_date FROM public.ckanext_opendstats_resource_downloads order by tracking_date DESC limit 1),'1970-01-01');
    '''
    conn = model.Session.connection()
    try:
        conn.execute(sa.sql.text(sql))
