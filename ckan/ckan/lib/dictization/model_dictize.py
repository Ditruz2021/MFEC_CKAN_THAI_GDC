...
def request_dictize(request, context):

    result_dict = d.table_dictize(request, context)
    
    id = result_dict.pop('id', None)
    notes = result_dict.pop('notes', None)
    owner_org = result_dict.pop('owner_org', None)
    modified = result_dict.pop('modified', None)
    first_name = result_dict.pop('first_name', None)
    package_name = result_dict.pop('package_name', None)
    email = result_dict.pop('email', None)
    scope = result_dict.pop('scope', None)
    send_data = result_dict.pop('send_data', None)
    objective = result_dict.pop('objective', None)
    last_name = result_dict.pop('last_name', None)
    created = result_dict.pop('created', None)

    result_dict['id'] = id
    result_dict['notes'] = notes
    result_dict['owner_org'] = owner_org
    result_dict['modified'] = modified
    result_dict['first_name'] = first_name
    result_dict['package_name'] = package_name
    result_dict['email'] = email
    result_dict['scope'] = scope
    result_dict['send_data'] = send_data
    result_dict['objective'] = objective
    result_dict['last_name'] = last_name
    result_dict['created'] = created
