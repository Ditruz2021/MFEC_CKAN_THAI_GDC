...
def package_request_create(context, data_dict):
    ...
    schema = package_plugin.create_package_request_schema()        
    ...
    data, errors = lib_plugins.plugin_request_validate(
        package_plugin, context, data_dict, schema, 'package_request_create')    

    if errors:
        model.Session.rollback()
        raise ValidationError(errors)

    pkg = model_save.package_request_dict_save(data, context)
