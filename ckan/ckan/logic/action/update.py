...
def update_news_count(context, data_dict):   
    id = data_dict.get('id')
    if id is None:
        raise ValidationError({'id': _('Missing value')})
    
    sql = '''
        UPDATE "ckanext_pages"
        SET "count" = "count" + 1
        WHERE id = :ids
    '''
    Session().execute(sql,{'ids': id})
    Session().commit()
    return {'success': True}

def package_request_update(context, data_dict):  
    model = context['model']
    _check_access('package_request_list', context, data_dict)

    id = data_dict.get('id') or data_dict.get('id')
    if id is None:
        raise ValidationError({'id': _('Missing value')})
    
    pkg = model.PackageRequest.get(id)
    if pkg is None:
        raise NotFound(_('Package was not found.'))
    
    schema = context.get('schema') or schema_.default_show_package_request_schema()
    ...
    package_request_table = model.package_request_table

    update = package_request_table.update()
    update = update.where(package_request_table.c.id == id)
    update = update.values(send_data = True)
...
def package_repair_update(context, data_dict):  
    ...
    pkg = model.Package.get_id(id) 
    if pkg is None:
        raise NotFound(_('Package was not found.'))
    
    schema = context.get('schema') or schema_.default_update_package_schema()
    ...
    update = package_table.update()
    update = update.where(package_table.c.id == id)
    update = update.values(state = 'active')
    ...
    if not pkg.private:
        ...
        activity = pkg.activity_stream_item('repair', user_id)
...
def group_repair_update(context, data_dict):  
    model = context['model']
    user = context['user']
    session = context['session']
    _check_access('package_request_list', context, data_dict)
    ...
    schema = context.get('schema') or schema_.default_group_repair_schema()
    ...
    update = group_table.update()
    update = update.where(group_table.c.id == id)
    update = update.values(state = 'active')

    conn = model.Session.connection()
    result = conn.execute(update)

    if pkg.is_organization:
    ...
    if activity_dict is not None:
        activity_dict['data'] = {
                'group': dictization.table_dictize(pkg, context)
                }
