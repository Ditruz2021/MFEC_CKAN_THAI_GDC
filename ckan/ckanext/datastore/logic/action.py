    ...
    **Results:**
    ...
    if g.user and data_api == 'True':
        ...
        dictized_package = ckan.logic.get_action('package_show')(
            context,
            {
                'id': dictized_resource.get('package_id'),
        ...
        activity_dict = {
            u'data':{
                u'resource': data_dict['resource_id'],
                u'actor': actor.name if actor else None,
                u'package':dictized_package,
            }, 
            u'user_id': user.id,
            u'object_id': dictized_resource.get('package_id'),
            u'activity_type': 'call datastore search',
        }
