...
def package_request_list(context, data_dict):
    model = context['model']
    ...
    query = model.Session.query(
            ...
            model.PackageRequest.id.label('id'),
            model.PackageRequest.notes.label('notes'),
            model.PackageRequest.owner_org.label('owner_org'),
            model.PackageRequest.first_name.label('first_name'),
            model.PackageRequest.last_name.label('last_name'),
            model.PackageRequest.modified.label('modified'),
            model.PackageRequest.package_name.label('package_name'),
            model.PackageRequest.email.label('email'),
            model.PackageRequest.scope.label('scope'),
            model.PackageRequest.send_data.label('send_data'),
            model.PackageRequest.objective.label('objective'),
            model.PackageRequest.created.label('created'),
        )        
    ...
    if id:
        ...
        if q:
            query = model.PackageRequest.search(q, query)

    _list = []
    
    for request in query.all():
            result_dict = model_dictize.request_dictize(request[0], context)
            _list.append(result_dict)            
...
FROM tracking_summary WHERE url in (SELECT ('/dataset/' || name) as url FROM package WHERE "state" = 'active') GROUP BY url ORDER BY count DESC limit 5;'''
    ...
    data = []
    for row in result:
        data.append({'url': row.url, 'count': row.count, 'name': row.name})
    return data

def total_group_review(context, data_dict):
    model = context['model']
...
(SELECT count(*) FROM "user" WHERE "user"."state" = 'active' AND "user".sysadmin = FALSE) as countuser;'''
    result = model.Session.execute(text(sql))
    data = []
    for row in result:
        data.append({'count': row.count,'countuser': row.countuser})

    return data

def total_news_review(context, data_dict):
    model = context['model']
    sql = u'''SELECT title as "name","count" FROM "ckanext_pages" ORDER BY "count" DESC limit 5;'''
    result = model.Session.execute(text(sql))
    data = []
    for row in result:
        data.append({'name': row.name, 'count': row.count})
...
def top_package(context, data_dict):
    model = context['model']
    _check_access('package_request_list', context, data_dict)    
    sql = u'''SELECT s.url,SUM(s.recent_views) as recent_views,(SELECT title FROM package WHERE name = REPLACE(s.url,'/dataset/','') AND "state" = 'active') as name,
(SELECT sum("count")  FROM tracking_summary t WHERE t.url in (SELECT ('/dataset/' || package.name || '/resource/' || resource."id") as url FROM package INNER JOIN resource ON package."id" =  resource.package_id WHERE package."state" = 'active') and t.url like s.url || '%' ) as download
FROM tracking_summary s WHERE s.url in (SELECT ('/dataset/' || name) as url FROM package WHERE "state" = 'active' ) GROUP BY url ORDER BY recent_views DESC;'''
    result = model.Session.execute(text(sql))
    data = []
    for row in result:
        data.append({'name': row.name, 'url': row.url,'recent_views': row.recent_views, 'download': row.download})

    return data

def org_package(context, data_dict):
    model = context['model']
    _check_access('package_request_list', context, data_dict)    
    sql = u'''SELECT s.url,SUM(s.recent_views) as views,
(SELECT title FROM package WHERE name = REPLACE(s.url,'/dataset/','') AND "state" = 'active') as name,
(SELECT sum("count")  
FROM tracking_summary t WHERE t.url in (SELECT ('/dataset/' || package.name || '/resource/' || resource."id") as url FROM package 
INNER JOIN resource ON package."id" =  resource.package_id WHERE package."state" = 'active') and t.url like s.url || '%' ) as download,
DATE_PART('YEAR',tracking_date) AS Years,
DATE_PART('MONTH',tracking_date ) AS Months,
(SELECT g.title FROM "package" p INNER JOIN "group" g ON p.owner_org = g."id" WHERE p."id" = package_id AND p."state" = 'active') AS OrgName,
(SELECT count(*) FROM "activity" a WHERE a.object_id = package_id AND a.activity_type = 'changed package') AS UpdateCount
FROM tracking_summary s WHERE s.url in (SELECT ('/dataset/' || name) as url FROM package WHERE "state" = 'active') 
GROUP BY url,Years,Months,package_id ORDER BY OrgName, Years DESC,Months;'''
    result = model.Session.execute(text(sql))
    data = []
    for row in result:
        data.append({'name': row.name, 'url': row.url,'views': row.views,
                      'download': row.download, 'years': int(row.years), 'months': int(row.months),
                      'orgname': row.orgname, 'updatecount': row.updatecount})
...
def package_request_show(context, data_dict):
    '''Return the metadata of a dataset (package) and its resources.
...
def all_activity_list(context, data_dict):
    ...
    activity_objects = model.activity.all_activity_list(
        limit=limit, offset=offset,
        include_hidden_activity=include_hidden_activity,
    )
    _objects = model_dictize.activity_list_dictize(
        activity_objects, context,
        include_data=data_dict['include_data'])
    
    model = context['model']
    sql = u'''SELECT id FROM "user" WHERE "name" = :name limit 1;'''
    result = model.Session.execute(text(sql),{'name': filter})
    data = []
    filtered = []
    for row in result:
    ...
    if data:
        _id = data[0]["id"]
        filtered = [filter_objects for filter_objects in _objects if filter_objects["user_id"] == _id]

    else:
        if filter != "":
            filtered = filter_by_title(filtered, filter)

    return filtered

def filter_by_title(data, title):
    filtered_data = [d for d in data if "data" in d and ("package" in d["data"] and d["data"]["package"].get("title") == title) or ("group" in d["data"] and d["data"]["group"].get("title") == title)]
