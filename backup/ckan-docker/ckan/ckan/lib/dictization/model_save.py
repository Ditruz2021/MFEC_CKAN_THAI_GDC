...
def package_request_dict_save(pkg_dict, context):
    model = context["model"]
    package = context.get("package_request")
    allow_partial_update = context.get("allow_partial_update", False)
    if package:
        pkg_dict["id"] = package.id
    Package = model.PackageRequest

    if 'created' in pkg_dict:
        del pkg_dict['created']

    pkg = d.table_dict_request_save(pkg_dict, Package, context)
