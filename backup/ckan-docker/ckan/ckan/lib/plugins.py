...
_package_plugins = {}
_package_request_plugins = {}
# The fallback behaviour
_default_package_plugin = None
_default_package_request_plugin = None
...
def reset_package_request_plugins():
    global _default_package_request_plugin
    _default_package_request_plugin = None
    global _package_request_plugins
    _package_request_plugins = {}
...
def lookup_package_request_plugin(package_type=None):
    """
    Returns the plugin controller associoated with the given package type.
    ...
    """
    if package_type is None:
        return _default_package_request_plugin
    return _package_request_plugins.get(package_type, _default_package_request_plugin)
...
def plugin_request_validate(plugin, context, data_dict, schema, action):
    """
    Backwards compatibility with 2.x dataset group and org plugins:
    return a default validate method if one has not been provided.
    """
    #base.abort(404, schema)
    # if hasattr(plugin, 'validate'):
    #     result = plugin.validate(context, data_dict, schema, action)
    #     base.abort(404, result)
    #     if result is not None:
    #         return result
    # base.abort(404, 'result')
    return toolkit.navl_validate(data_dict, schema, context)
...
class DefaultDatasetForm(object):
    ...
    def create_package_request_schema(self):
        return logic.schema.default_create_package_request_schema()
